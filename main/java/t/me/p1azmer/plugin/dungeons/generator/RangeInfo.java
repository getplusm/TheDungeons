...
public class RangeInfo {
    ...
    public RangeInfo(int startX, int startZ, int distanceMin, int distanceMax,
    ...
    public static RangeInfo read(@NotNull JYML cfg, @NotNull String path) {
        ...
        List<Material> materials = cfg.getStringList(path + ".Material.List").stream().map(sId -> StringUtil.getEnum(sId, Material.class).orElse(null)).filter(material -> {
            if (material == null) {
        ...
        List<Biome> biomes = cfg.getStringList(path + ".Biome.List").stream().map(sId -> StringUtil.getEnum(sId, Biome.class).orElse(null)).filter(biome -> {
            if (biome == null) {
        ...
        return new RangeInfo(startX, startZ, distanceMin, distanceMax, materials, biomes, biomesAsBlack, materialsAsBlack);
    }
