...
public abstract class AbstractCommand<P extends NexPlugin<P>> implements IPlaceholder {
    ...
    public List<String> getTab(@NotNull Player player, int arg, @NotNull String[] args) {
        return this.getCommandFlags().stream().map(CommandFlag::getNamePrefixed).collect(Collectors.toList());
    }
    ...
    protected final void printUsage(@NotNull CommandSender sender) {
        plugin.getMessage(CoreLang.CORE_COMMAND_USAGE).replace(this.replacePlaceholders()).send(sender);
    }

    protected final void errorPermission(@NotNull CommandSender sender) {
        plugin.getMessage(CoreLang.ERROR_PERMISSION_DENY).send(sender);
    }

    protected final void errorItem(@NotNull CommandSender sender) {
        plugin.getMessage(CoreLang.ERROR_ITEM_INVALID).send(sender);
    }

    protected final void errorPlayer(@NotNull CommandSender sender) {
        plugin.getMessage(CoreLang.ERROR_PLAYER_INVALID).send(sender);
    }

    protected final void errorSender(@NotNull CommandSender sender) {
        plugin.getMessage(CoreLang.ERROR_COMMAND_SENDER).send(sender);
    }

    protected final void errorNumber(@NotNull CommandSender sender, @NotNull String from) {
        plugin.getMessage(CoreLang.ERROR_NUMBER_INVALID).replace("%num%", from).send(sender);
    }
}