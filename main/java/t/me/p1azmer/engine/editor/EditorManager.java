...
//        boolean fixCommand = Version.isAbove(Version.V1_18_R2);
    ...
    public static void displayValues(@NotNull Player player, boolean autoRun, int page) {
        ...
        String header = ChatColor.GOLD + StringUtils.repeat("=", 8) + "[ " + ChatColor.YELLOW + "Помощь по значениям" + ChatColor.GOLD + " ]" + StringUtils.repeat("=", 8);
        ...
        items.forEach(item -> {
            ...
            component.showText(ChatColor.GRAY + "Нажмите, чтобы выбрать " + ChatColor.AQUA + item);
        ...
        if (!isFirstPage)
            message.addComponent("[<]", ChatColor.RED + "[<]").showText(ChatColor.GRAY + "Предыдущая страница").runCommand("/" + VALUES + " " + (page - 1) + " " + autoRun);
        if (!isLastPage)
            message.addComponent("[>]", ChatColor.RED + "[>]").showText(ChatColor.GRAY + "Следующая страница").runCommand("/" + VALUES + " " + (page + 1) + " " + autoRun);
