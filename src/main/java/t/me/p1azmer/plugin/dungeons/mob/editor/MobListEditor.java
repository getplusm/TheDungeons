...
public class MobListEditor extends EditorMenu<DungeonPlugin, MobManager> implements AutoPaged<MobConfig> {
    ...
    public MobListEditor(@NotNull MobManager mobManager) {
        ...
        this.addReturn(39).setClick((viewer, event) -> this.plugin.getEditor().openNextTick(viewer, 1));
        this.addNextPage(44);
        this.addPreviousPage(36);

        this.addCreation(EditorLocales.MOB_CREATE, 41).setClick((viewer, event) -> this.handleInput(viewer, Lang.Editor_Mob_Enter_Create, wrapper -> {
    ...
    public ItemStack getObjectStack(@NotNull Player player, @NotNull MobConfig mob) {
        ...
        if (Version.isBehind(Version.MC_1_20_6)) {
            if (mob.getEntityType() == EntityType.valueOf("MUSHROOM_COW")) material = Material.MOOSHROOM_SPAWN_EGG;
        } else {
            if (mob.getEntityType() == EntityType.valueOf("MOOSHROOM")) material = Material.MOOSHROOM_SPAWN_EGG;
        }
