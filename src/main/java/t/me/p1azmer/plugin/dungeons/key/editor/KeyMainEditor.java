...
public class KeyMainEditor extends EditorMenu<DungeonPlugin, Key> {
    ...
    public KeyMainEditor(@NotNull Key key) {
        ...
        this.addReturn(22).setClick((viewer, event) -> this.plugin.runTask(task -> plugin.getEditor().getKeysEditor().open(viewer.getPlayer(), 1)));

        this.addItem(Material.NAME_TAG, EditorLocales.KEY_NAME, 12).setClick((viewer, event) -> this.handleInput(viewer, Lang.EDITOR_ENTER_DISPLAY_NAME, wrapper -> {
