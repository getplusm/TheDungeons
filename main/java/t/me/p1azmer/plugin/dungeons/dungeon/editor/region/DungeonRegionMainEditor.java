...
public class DungeonRegionMainEditor extends EditorMenu<DungeonPlugin, DungeonRegion> {
    ...
    public DungeonRegionMainEditor(@NotNull DungeonRegion region) {
        ...
        this.addItem(region.isIgnoreAirBlocks() ? Material.BIRCH_BUTTON : Material.OAK_BUTTON, EditorLocales.REGION_IGNORE_AIR_BLOCKS, 8).setClick((viewer, event) -> {
            region.setIgnoreAirBlocks(!region.isIgnoreAirBlocks());
            this.save(viewer);
        });
        ...
        this.getItems().forEach(menuItem -> {
            if (menuItem.getOptions().getDisplayModifier() == null) {
                menuItem.getOptions().setDisplayModifier(((viewer, item) -> {
                    ...
                    if (item.getType().equals(Material.BIRCH_BUTTON) && !region.isIgnoreAirBlocks())
                        item.setType(Material.OAK_BUTTON);
                    else if (item.getType().equals(Material.OAK_BUTTON) && region.isIgnoreAirBlocks())
                        item.setType(Material.BIRCH_BUTTON);
