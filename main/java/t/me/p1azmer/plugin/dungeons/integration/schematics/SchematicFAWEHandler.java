    ...
    public boolean paste(@NotNull Dungeon dungeon, @NotNull File schematicFile) {
        ...
        try (EditSession editSession = this.worldEdit.getEditSessionFactory().getEditSession(weWorld, -1, actor)) {
            ...
            Operation operation = session.getClipboard()
                    ...
                    .to(toVector)
                    .ignoreAirBlocks(dungeon.getDungeonRegion().isIgnoreAirBlocks())
                    .copyEntities(true)
                    .build();
            ...
            plugin.error("Paste method take=" + (System.currentTimeMillis() - ms) + "ms");
            return true;
        ...
        for (int x = minPoint.getBlockX(); x <= maxPoint.getBlockX(); x++) {
            for (int y = minPoint.getBlockY(); y <= maxPoint.getBlockY(); y++) {
                for (int z = minPoint.getBlockZ(); z <= maxPoint.getBlockZ(); z++) {
                    ...
                    BlockType chestBlock = BlockTypes.get("minecraft:" + dungeon.getSettings().getChestMaterial().name().toLowerCase(Locale.ROOT));
