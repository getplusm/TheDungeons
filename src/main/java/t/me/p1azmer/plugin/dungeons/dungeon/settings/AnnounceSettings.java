...
public class AnnounceSettings extends AbstractSettings implements IPlaceholderMap {
    ...
    @NotNull
    public List<Announce> getAnnounces(@NotNull DungeonStage stage, int time) {
        return this.getAnnounceMap().entrySet().stream()
                .filter(entry -> entry.getKey().equals(stage) && entry.getValue().entrySet().stream().anyMatch(f -> Arrays.stream(f.getValue()).anyMatch(i -> i == time)))
