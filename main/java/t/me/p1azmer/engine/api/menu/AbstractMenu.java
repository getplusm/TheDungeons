    ...
    private class PutAnimation {
        ...
        public PutAnimation(JYML cfg) {
            this.animationType = JOption.create("Put_Animation.Type", PutItemAnimation.class, PutItemAnimation.NONE,
                    "Анимация появления предметов в меню",
                    "Доступные значения:",
                    PutItemAnimation.NONE + " - Обычное заполнение",
                    PutItemAnimation.LEFT_TO_RIGHT + " - Из левого столба в правый",
                    PutItemAnimation.RIGHT_TO_LEFT + " - Из правого столба в левый",
                    PutItemAnimation.UP_TO_DOWN + " - С верху вниз",
                    PutItemAnimation.DOWN_TO_UP + " - Снизу вверх"
            ).read(cfg);
            this.delay = JOption.create("Put_Animation.Delay", 3L,
                    "Задержка между добавлением каждого предмета"
            ).read(cfg);
            this.maxTime = JOption.create("Put_Animation.Max_Time", 2,
                    "Если предметы не установились за это время, то мы сразу добавляем все предметы без задержек"
            ).read(cfg);
        }
