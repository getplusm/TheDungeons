...
public class AnnounceMainEditor extends EditorMenu<DungeonPlugin, Announce> {
    ...
    public AnnounceMainEditor(@NotNull Announce announce) {
        ...
        this.addReturn(8).setClick((viewer, event) -> this.plugin.runTask(task -> announce.getManager().getEditor().open(viewer.getPlayer(), 1)));

        this.addItem(Material.COMPARATOR, EditorLocales.ANNOUNCE_MESSAGES, 0).setClick((viewer, event) -> this.handleInput(viewer, Lang.EDITOR_REWARD_ENTER_CHANCE, wrapper -> {
        ...
        this.getItems().forEach(menuItem -> menuItem.getOptions().addDisplayModifier(((viewer, item) -> ItemReplacer.replace(item, announce.replacePlaceholders()))));
    }
