...
public class SchematicFAWEHandler implements SchematicHandler {
    ...
    ServerImplementation syncThread;

    Map<Location, EditSession> editSessionMap = Maps.newConcurrentMap();
    Map<Dungeon, Location> placedMap = Maps.newConcurrentMap();

    public SchematicFAWEHandler(JavaPlugin plugin, ServerImplementation syncThread) {
        this.log = plugin.getLogger();
        this.syncThread = syncThread;
        this.sessionConsole = new SessionConsole(plugin);
    }
    ...
    @Override
    public boolean paste(Dungeon dungeon, @NotNull File schematicFile) {
        Location location = dungeon.getLocation();
        if (location == null) {
            log.severe("Dungeon '" + dungeon.getId() + "' has no location!");
            return false;
        }

        World world = location.getWorld();
        if (!undo(dungeon)) {
            log.severe("Dungeon '" + dungeon.getId() + "' cannot undo");
            return false;
        }
        ...
        try (EditSession editSession = WORLD_EDIT.newEditSession(worldEditWorld)) {
            editSession.setReorderMode(EditSession.ReorderMode.FAST);
            Operation operation = holder.createPaste(editSession)
                    .to(toVector)
                    .ignoreAirBlocks(settings.isIgnoreAirBlocks())
                    .copyEntities(true)
                    .build();
            ...
            Logger.getGlobal().severe("pasted " + schematicFile.getName());
            return true;
        } catch (WorldEditException e) {
            log.log(Level.SEVERE, "Got exception when paste the schematic at '" + dungeon.getId() + "' dungeon!", e);
            return false;
        }
    }
    ...
    public boolean undo(@NotNull Dungeon dungeon) {
        ...
        try (EditSession newEditSession = sessionBuilder.blockBag(blockBag).actor(actor).world(editSession.getWorld()).build()) {
            ...
            WORLD_EDIT.flushBlockBag(actor, editSession);
            placedMap.remove(dungeon, location);
            return true;
        } catch (Exception exception) {
            log.log(Level.SEVERE, "Got exception when undo the schematic at '" + dungeon.getId() + "' dungeon!", exception);
            return false;
        }
    }

    @Override
    public int getChestBlockCount(@NotNull Dungeon dungeon, @NotNull File schematicFile) {
        ClipboardFormat clipboardFormat = ClipboardFormats.findByFile(schematicFile);
        ...
        try {
            ...
            log.log(Level.SEVERE, "Got exception while trying to get amount of chest blocks in schematic: " + schematicFile.getName(), exception);
            return 0;
        }
        Clipboard clipboard = holder.getClipboard();
        if (clipboard == null) {
            log.severe("The schematic module returned the following error: Clipboard for file not found or invalid");
            return 0;
        }
        BlockVector3 minPoint = clipboard.getMinimumPoint();
        BlockVector3 maxPoint = clipboard.getMaximumPoint();
        int amount = 0;
        Material material = Material.BARREL;// TODO dungeon.getChestSettings().getMaterial();
    ...
    private record SchematicLoadTask(File file, ClipboardFormat format) implements Callable<ClipboardHolder> {

        private static final Map<File, ClipboardHolder> HOLDERS = Maps.newHashMap();

        @Override
        public ClipboardHolder call() throws Exception {
            ClipboardHolder holder = HOLDERS.get(file());
            if (holder == null)
